...
# HoneyCloud — Collector Service
# Tails cowrie.json → scores each event → writes to PostgreSQL
# Includes Self-Healing for NeonDB Serverless Timeouts
#
# Usage:
#   python -m honeycloud.collector                  # production
#   python -m honeycloud.collector --mock           # local test
...
def tail_and_collect(log_path: str, db_url: str, dry_run: bool = False,
                     from_start: bool = False):
    """
    Tail cowrie.json and process new lines as they arrive.
    Includes NeonDB Serverless self-healing connection logic.
    """
    log.info(f"📂 Tailing: {log_path}")
    log.info(f"💾 DB write: {'disabled (dry run)' if dry_run else 'enabled'}")
    if from_start:
    ...
    # Initialize the first connection inside the loop manager
    if not dry_run:
    ...
    try:
        while True:
                ...
                for line in lines:
                    ...
                    if dry_run:
                        ...
                        try:
                            ...
                            log.warning(f"⚠️ DB connection dropped by NeonDB. Reconnecting... ({e})")
                            # Self-Healing Logic: Close dead connection, open a fresh one, and retry
                            try:
                                conn.close()
                            except:
                                pass
                            try:
                                ...
                                _upsert_session(conn, event, session_state)
                                events_written += 1
                                log.info(f"✅ [RECOVERED] {eid:<30}  {src_ip:<18}")
                            except Exception as fatal_e:
                                log.error(f"❌ Fatal DB drop, event lost: {fatal_e}")
            ...
            now = time.time()
            if now - last_heartbeat >= HEARTBEAT_INTERVAL:
                # Proactive Keepalive: Ping the DB so NeonDB doesn't sleep
                if not dry_run and conn:
                    try:
                        with conn.cursor() as ping_cur:
                            ping_cur.execute("SELECT 1;")
                        log.info("💓 Heartbeat & DB Keepalive: OK")
                    except Exception as ping_e:
                        log.warning(f"⚠️ Heartbeat Keepalive failed: {ping_e}")
                        # Don't worry; the Reactive Fallback in the main loop 
                        # will catch and fix this when the next real attack arrives.

                log.info(
                    f"📊 Status — processed={events_processed}  "
                    f"written={events_written}  "
                    f"watching_byte={last_pos}"
                )
                last_heartbeat = now
...
def main():
    ...
    if not log_path:
        log.error("cowrie.json not found after waiting 60 seconds.")
        return

    if args.dry_run:
        log.info("🧪 Dry run mode — no DB writes")
        tail_and_collect(log_path, db_url=args.db_url, dry_run=True,
                         from_start=args.from_start)
        return

    log.info("🔌 Verifying DB Schema...")
    init_conn = _connect(args.db_url)
    _ensure_schema(init_conn)
    init_conn.close() # Close initial schema connection so it doesn't idle out

    try:
        tail_and_collect(log_path, db_url=args.db_url, dry_run=False,
                         from_start=args.from_start)
    except KeyboardInterrupt:
        log.info("Collector stopped.")
